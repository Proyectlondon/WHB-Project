...
function renderAudio(audio) {
  ...
  const setQueue = (nextMode) => {
    ...
    if (nextMode === 'album' && activeIndex >= 0) {
      ...
      queueIndices = activeIndex >= 0 ? [activeIndex] : [];
    }
    mode.textContent = nextMode === 'album' ? `Álbum · ${audio[queueIndices[0]]?.album || ''}` : nextMode === 'all' ? 'Todo el catálogo' : 'Termina al finalizar';

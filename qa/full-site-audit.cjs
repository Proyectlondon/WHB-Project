...
async function functionalJourney(browser) {
  ...
  check(catalog.audio.length === 46, `Catálogo de audio: ${catalog.audio.length}/46`, findings);
  ...
  const anthology = page.locator('.album-block').filter({ has: page.locator('summary', { hasText: 'ANTHOLOGY VOL II' }) }).first();
  if (!(await anthology.evaluate((details) => details.open))) await anthology.locator('summary').click();
  await anthology.locator('[data-audio-index]', { hasText: 'Caminos de Zipacón' }).first().click();
  ...
  check(miniTitle === '40 días después', `El mini reproductor muestra “${miniTitle}” tras iniciar el álbum`, findings);
